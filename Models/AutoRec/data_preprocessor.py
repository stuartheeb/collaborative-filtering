...
def read_rating(path, num_users, num_items,num_total_ratings, a, b, train_ratio):
    ...
    for line in lines:
        ...
        user_idx = int(user)
        item_idx = int(item)
        val_R[user_idx, item_idx] = int(rating)
        val_mask_R[user_idx, item_idx] = 1
        user_val_set.add(user_idx)
        item_val_set.add(item_idx)

    return R, mask_R, C, train_R, train_mask_R, test_R, test_mask_R,val_R,val_mask_R,num_train_ratings,num_test_ratings,\
